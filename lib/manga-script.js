...
exports.getIndexesForAllMangaJson = getIndexesForAllMangaJson;
exports.downloadImagesForAllManga = downloadImagesForAllManga;
exports.setup = setup;
    ...
    promise.then(function (manga_to_process) {
        ...
        manga_to_process.forEach(function(manga_url){
            (function(mga_url) {
                asyncTasks.push(
                    function(callback) {
                        getMangaJson(mga_url, opts, function(done) {
                            console.log(mga_url + ' done: ' + done );
        ...
        async.series(asyncTasks, function(err, results) {
            console.log('Errors:');
            if (err) console.log(err); else console.log('None.');
            console.log('Results:');
            console.log(results);
...
function downloadImagesForAllManga(opts) {
    var manga_json_dir = 'manga_json';
    var overwrite = false;
    ...
    manga_json_files.forEach(function(manga_json_file) {
        ...
        (function(mga_json_f) {
            asyncTasks.push(
                function(callback) {
                    manga_downloader.downloadManga(mga_json_f, opts, function(done) {
                        callback(null, done);
                    });
                }
...
/**
 * Must have downloaded images for the manga for this to work.
 *
 * @param opts
 */
function getIndexesForAllMangaJson(opts) {
    var manga_json_dir = 'manga_json';
    var overwrite = false;
    ...
    manga_json_files.forEach(function(manga_json_file) {
        ...
        (function(mga_json_f) {
            asyncTasks.push(
                function(callback) {
                    getMangaIndexJson(mga_json_f, opts, function(done) {
                        callback(null, {'manga_json_file': mga_json_f, 'done': done});
                    })
                }
...
function getMangaJson(manga_url, opts, callback) {
    ...
    .spread( function (urls, titles) {
        ...
        return Promise.map(urls, function(url) {
            return mfs.getPageNumbersPromise(url);
        }, {concurrency: 5}).then( function (page_numbers) { // Passed Down.
            //Debug
            ...
            if (urls.length === page_numbers.length) {
                for (var i = 0; i < page_numbers.length; i++) {
                    ...
                    url.pop()
                    url = url.join('/') + '/'; // Remove 1.html from http://mangafox.me/manga/azure_dream/v01/c007/1.html then join()
                    // Debug
                    //console.log(url);

                    // Set chapter urls.
                    temp_page_urls.push(url) // ==> ... http://mangafox.me/manga/azure_dream/v01/c007/ ...
                }
            } else {
            ...
            for (i = 0; i < temp_page_urls.length; i++) { // chapter_urls.length == page_numbers.length.
                ...
                for (var j = 0; j < page_numbers[i].length; j++) {
                    // Debug
                    //console.log(temp_page_urls[i] + page_numbers[i][j] + ext);
    ...
    .catch(function(err) {
        console.log(err);
        console.log("Check the url if it is correct? url: " + manga_url);
    });
}
...
/**
 * Create folders and default config file.
 *
 *
 */
function setup() {
    var defaultOpts = {
        ...
        'manga_index_directory': 'manga_index',
    ...
    //Debug
    //console.log(defaultOpts['json_directory']);
    //console.log(defaultOpts['manga_directory']);
    //console.log(defaultOpts['manga_index_directory']);

    manga_file.makeDir(defaultOpts['json_directory']);
    manga_file.makeDir(defaultOpts['manga_directory']);
    manga_file.makeDir(defaultOpts['manga_index_directory']);

    manga_file.saveFileSync('config.json', defaultOpts);
}
