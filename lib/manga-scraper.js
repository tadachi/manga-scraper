...
exports.getNonDuplicates = getNonDuplicates;
...
function updateManga(json_file) {
    var mfs = new MangaFoxScraper();
    var manga_json = loadJSON(json_file);
    console.log(manga_json);
    console.log(manga_json['manga_url']);
    var chapter_urls_promise = mfs.getChapterUrlsPromise(manga_json['manga_url']);


    chapter_urls_promise.then( function(chapter_urls) {

        var old_chapters = manga_json['chapter_urls']
        var new_chapters = chapter_urls['urls'].sort();
        new_chapters.push('http://mangafox.me/manga/macchi_shoujo/v01/c002/1.html');
        // Debug.
        //console.log(old_chapters);
        //console.log(new_chapters );
        var updated_chapters = getNonDuplicates(old_chapters, new_chapters);
    ...
    if (array1.length == array2.length) {
        ...
        try {
            if (array2.length > array1.length) {
                // Make sure every item up to the old array is accounted for.
                for (var i = 0; i < array1.length; i++) {
                    if (array1[i] != array2[i]) {
                        var args = {'array1': array1, 'array2': array2};
                        throw new ArraysDoNotMatch('Arrays do not match.', args);
                    }
                }

                // Return the new items from the new array.
                ...
                throw new Array2LenMustBeGreaterException('array2 must have greater length than array1.', args);
            }
        } catch(err) {
            console.log(err);
        }
