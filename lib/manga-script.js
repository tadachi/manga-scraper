...
function getMangaIndexJson(json_file, opts, callback) {
    ...
    for (var i = 0; i < volumes.length; i++) {
        // Add volume.
        if (!manga_index_json[volumes[i]]) manga_index_json[volumes[i]] = {};
        chapters = fs.readdirSync(path.join(manga_dir, manga_name, volumes[i])); // manga/another_world_it_exists/v01/ .. /v02/ ..

        for (var j = 0; j < chapters.length; j++) {
            if (!manga_index_json[volumes[i]][chapters[j]]) manga_index_json[volumes[i]][chapters[j]] = {};
            ...
            manga_index_json[volumes[i]][chapters[j]]['img'] = full_path_images;
            manga_index_json[volumes[i]][chapters[j]]['title'] = title;
